...
router.post('/login', async (req, res) => {
  ...
  try {
    ...
    if (user) {
      const token = jwt.sign({ type: 'Academic', profile: user }, jwtSecret, { expiresIn: '1h' });
    ...
    if (user) {
      const token = jwt.sign({ type: 'Industry', profile: user }, jwtSecret, { expiresIn: '1h' });
      return res.json({ token });
    }
    ...
    if (user) {
      const token = jwt.sign({ type: 'Admin', profile: user }, jwtSecret, { expiresIn: '1h' });
      return res.json({ token });
    }
